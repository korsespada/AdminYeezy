...
export async function recognizeMeasurementTemplateAction(sourceImageUrl: string) {
  ...
  try {
    ...
    const output = await runBatchAiOpenRouter({
      settings,
      systemPrompt: `Ты распознаёшь ТОЛЬКО размерные таблицы на изображениях. Не описывай товар и не добавляй отсутствующие значения. Верни строгий JSON без Markdown: {"unit":"см","columns":[{"key":"waist","label":"Талия"}],"rows":[{"size":"S","values":{"waist":"72"}}],"note":""}. Ключи колонок — короткие латинские snake_case. В values переписывай только одно точное числовое значение из ячейки таблицы: нельзя превращать допуск, примечание или диапазон в значение ячейки. Текст вне сетки, включая «ручное измерение», «допуск/погрешность 1–2 см», «±1–2 см» и аналогичные указания, переноси целиком в note. Например, при ячейке «86» и подписи «погрешность 1–2 см» верни values: {"waist":"86"}, note: "Допуск ручного измерения: 1–2 см"; никогда не возвращай «86–90 см» в values. Если ячейка нечитаема, оставь пустую строку.`,
      userPrompt: 'На изображении размерная сетка. Перенеси видимые колонки, размеры и точные значения ячеек в JSON. Не угадывай и не дополняй значения. Отдельные подписи о допуске, погрешности и ручном измерении пиши только в note, не смешивая их с ячейками.',
      contactSheets: [imageUrl],
    })
