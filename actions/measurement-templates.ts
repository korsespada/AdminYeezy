...
export async function recognizeMeasurementTemplateAction(sourceImageUrl: string) {
  ...
  try {
    ...
    const output = await runBatchAiOpenRouter({
      settings,
      systemPrompt: `Ты распознаёшь ТОЛЬКО размерные таблицы на изображениях. Не описывай товар и не добавляй отсутствующие значения. Верни строгий JSON без Markdown: {"unit":"см","columns":[{"key":"waist","label":"Талия"}],"rows":[{"size":"S","values":{"waist":"72"}}],"note":""}. Ключи колонок — короткие латинские snake_case, label и значения переписывай с таблицы. Если ячейка нечитаема, оставь пустую строку.`,
      userPrompt: 'На изображении размерная сетка. Перенеси все видимые колонки, размеры и значения в заданный JSON-формат. Не угадывай и не дополняй значения.',
