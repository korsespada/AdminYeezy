...
import ChromoffAiProductDrawer from '@/components/chromoff/ChromoffAiProductDrawer'
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-5 text-slate-100 sm:p-7">
      <div className="mx-auto max-w-[1500px]">
        ...
        {tab === 'products' && <section>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {listings.map((listing) => {
              ...
              return <div key={listing.id} role="button" tabIndex={0} onClick={() => setViewingId(listing.id)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); setViewingId(listing.id) } }} className={`flex min-h-32 cursor-pointer gap-3 rounded-xl border p-3 text-left transition ${checked ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-800 bg-slate-900 hover:border-slate-700'}`}>
                ...
                <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h2 className="line-clamp-2 font-medium">{listing.brand?.name ? `${listing.brand.name} ` : ''}{listing.name}</h2><button type="button" title={checked ? 'Снять выбор' : 'Выбрать для AI'} onClick={(event) => { event.stopPropagation(); setSelected((current) => checked ? current.filter((id) => id !== listing.id) : [...current, listing.id]) }} className="-m-1 shrink-0 rounded p-1 hover:bg-slate-800">{checked ? <CheckSquare className="h-5 w-5 text-emerald-400" /> : <Square className="h-5 w-5 text-slate-600" />}</button></div><p className="mt-2 text-xs text-slate-400">{listing.chromoff_category?.name || 'Без категории'} · {formatPrice(listing.price_cents)}</p><div className="mt-3 flex flex-wrap gap-1.5"><Badge variant="outline" className={listing.h1 && listing.seo_description ? 'border-emerald-600/40 text-emerald-300' : 'border-amber-600/40 text-amber-300'}>{listing.h1 && listing.seo_description ? 'SEO заполнено' : 'Нужно SEO'}</Badge>{runItem && <Badge variant="outline" title={runItem.error_message || ''}>{statusLabels[runItem.status]}</Badge>}</div></div>
              </div>
            })}
          </div>
        ...
        <ChromoffAiProductDrawer listing={viewingId ? listings.find((listing) => listing.id === viewingId) || null : null} isOpen={Boolean(viewingId)} onClose={() => setViewingId(null)} queueItem={viewingId ? itemByListing.get(viewingId) : undefined} />
      </div>
    </main>
  )
}
