  ...
  return (
    <div className="fixed inset-0 z-[120] bg-slate-950/90 p-3 backdrop-blur-sm">
      <div className="mx-auto flex max-h-[calc(100vh-1.5rem)] max-w-6xl flex-col overflow-hidden rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
        ...
        <footer className="flex shrink-0 gap-3 border-t border-slate-700 bg-slate-900 p-3"><button onClick={add} className="inline-flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-200"><Plus className="h-4 w-4" /> Добавить</button><button onClick={save} disabled={pending} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" /> Сохранить правила</button><span className="ml-auto self-center text-xs text-slate-500">{rules.length} правил</span></footer>
      </div>
    </div>
  )
}
