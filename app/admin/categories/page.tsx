...
import { Collections, type Category, type Product } from '@/lib/types'
...
export default async function CategoriesPage({
  ...
  try {
    ...
    // Fetch categories directly from the categories collection (much faster!)
    let filter = ''
    if (searchTerm) {
      filter = `name ~ "${searchTerm}" || description ~ "${searchTerm}"`
    }
    
    const result = await pb.collection(Collections.Category).getList<Category>(page, perPage, {
      sort: 'name',
      filter: filter,
      requestKey: null,
    })
    ...
    error = `Failed to load categories: ${err?.message || 'Unknown error'}`
  }
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 sm:px-6 rounded-lg">
              <div className="flex flex-1 justify-between sm:hidden">
                <a
                  href={page > 1 ? `/admin/categories?page=${page - 1}` : '#'}
                ...
                <a
                  href={page < totalPages ? `/admin/categories?page=${page + 1}` : '#'}
