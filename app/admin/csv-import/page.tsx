    ...
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans">
      ...
      <div className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-30 shadow-lg">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end flex-wrap">
            ...
            {saveMsg && (
              <span
                className={`text-xs px-3 py-1 rounded-full ${saveMsg.startsWith("✓") ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
              >
                {saveMsg}
              </span>
            )}
                ...
                >
                  {isPushing ? (
                    <>
                      ...
                      {pushProgress
                        ? `Загружаю ${pushProgress.current}/${pushProgress.total}...`
                        : "Загружаю..."}
                    </>
                  ) : (
                    <>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {result && (
          <div
            id="import-result"
            className={`mb-6 p-5 rounded-2xl border shadow-xl animate-in fade-in slide-in-from-top-4 duration-500 ${
              result.failed === 0
        ...
        {products.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 pb-32">
            {filteredProducts.map((product) => {
              ...
              return (
                <CsvProductCard
                  key={`${product.productId}-${realIndex}`}
                  product={product}
                  index={realIndex}
                  lookups={lookups}
    ...
    >
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-700 mt-auto">
          ...
          <span
            className={`text-xs px-2 py-1 rounded-full ${product.status === "active" ? "bg-emerald-900/50 text-emerald-400" : "bg-slate-700 text-slate-400"}`}
          >
            {product.status}
          </span>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-800">
                <div className="space-y-1">
                  <label className="text-xs text-slate-500">
                    Brand ({resolveName(local.brand, lookups?.brands || [])})
                  </label>
                  <input
                    type="text"
                    value={local.brand}
                ...
                <div className="space-y-1">
                  <label className="text-xs text-slate-500">
                    Category (
                    {resolveName(local.category, lookups?.categories || [])})
                  </label>
                  <input
                    type="text"
