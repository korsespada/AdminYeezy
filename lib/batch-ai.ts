...
- description: не используй единый лимит длины для всех товаров. Следуй переданному в запросе «Профилю исходных данных для описания»: при богатом фактическом источнике сохрани все подтверждённые технические детали, при скудном источнике опирайся на фотографии и не заполняй текст догадками. Разделяй смысловые части не более чем двумя одиночными переносами.
...
- Следуй переданному в запросе «Профилю исходных данных для описания»: он задаёт ожидаемую глубину и целевой объём. Не сокращай богатый фактический источник до общих фраз и не раздувай скудный текст неподтверждёнными деталями.
...
export type BatchAiDescriptionEvidenceProfile = {
  mode: 'rich_source' | 'balanced_source' | 'photo_first' | 'sparse_source'
  sourceTextLength: number
  technicalDetailCount: number
  photoCount: number
}

const SOURCE_TECHNICAL_DETAIL_RE = /(?:\b(?:tpu|eva|gore[- ]?tex|vibram|boost|air|max|foam|leather|suede|mesh|knit|canvas|nylon|wool|cotton|silk|zipper|lace|buckle|hardware|lining|midsole|outsole|upper|heel|platform|quilted|embroid(?:ery|ered)?)\b|鞋面|中底|大底|鞋底|内里|材质|面料|皮革|真皮|牛皮|羊皮|麂皮|反绒|织物|网面|针织|帆布|尼龙|羊毛|棉|丝|拉链|纽扣|五金|刺绣|压纹|绗缝|跟高|鞋跟|后跟|鞋头|鞋带|肩带|手柄|内袋|口袋|подошв[а-я]*|верх[а-я]*|подкладк[а-я]*|кож[а-я]*|замш[а-я]*|текстил[а-я]*|трикотаж[а-я]*|фурнитур[а-я]*|молни[яей]|пуговиц[а-я]*|шнуровк[а-я]*|каблук[а-я]*|мыс[а-я]*|ремеш[а-я]*|карман[а-я]*|вышивк[а-я]*|стёжк[а-я]*|амортизац[а-я]*|протектор[а-я]*)/giu
const SOURCE_SIZE_TAIL_RE = /(?:\bsize\b|尺码|码数)\s*[:：]?.*$/gimu

function sourceDescriptionForEvidence(product: any) {
  return String(product?.description || '')
    .replace(SOURCE_SIZE_TAIL_RE, ' ')
    .replace(/\s+/gu, ' ')
    .trim()
}

export function batchAiDescriptionEvidenceProfile(product: any): BatchAiDescriptionEvidenceProfile {
  const sourceText = sourceDescriptionForEvidence(product)
  const technicalDetails = new Set(
    [...sourceText.matchAll(SOURCE_TECHNICAL_DETAIL_RE)]
      .map((match) => String(match[0] || '').toLocaleLowerCase('ru-RU').trim())
      .filter(Boolean),
  )
  ...
  // Один китайский иероглиф содержит примерно столько же фактической информации,
  // сколько несколько латинских букв; не считаем такой источник «коротким» только
  // из-за посимвольной длины.
  const sourceEvidenceLength = sourceTextLength + (sourceText.match(/[\u4e00-\u9fff]/gu) || []).length
  const technicalDetailCount = technicalDetails.size
  const photoCount = photos.length

  if (
    (sourceEvidenceLength >= 150 && technicalDetailCount >= 5)
    || (sourceEvidenceLength >= 220 && technicalDetailCount >= 3)
    || (sourceEvidenceLength >= 420 && technicalDetailCount >= 2)
  ) {
    return { mode: 'rich_source', sourceTextLength, technicalDetailCount, photoCount }
  }
  if ((sourceEvidenceLength >= 110 && technicalDetailCount >= 1) || sourceEvidenceLength >= 280) {
    return { mode: 'balanced_source', sourceTextLength, technicalDetailCount, photoCount }
  }
  if (photoCount > 0) return { mode: 'photo_first', sourceTextLength, technicalDetailCount, photoCount }
  return { mode: 'sparse_source', sourceTextLength, technicalDetailCount, photoCount }
}

export function buildBatchAiDescriptionEvidencePrompt(product: any) {
  const profile = batchAiDescriptionEvidenceProfile(product)
  const evidence = `Исходный текст: ${profile.sourceTextLength} знаков после исключения размерного ряда; технических сигналов: ${profile.technicalDetailCount}; фотографий: ${profile.photoCount}.`
  if (profile.mode === 'rich_source') {
    return [
      'Профиль исходных данных для описания: богатый фактический источник.',
      evidence,
      'Целевой объём description: 500–800 знаков. Сначала внутренне выдели все подтверждённые конкретные факты из текста и фотографий; сохрани технологии, материалы, конструкцию, точные цвета, расположение деталей и маркировки. Убирай только рекламу, неподтверждённые обещания и повторы. Не заменяй несколько конкретных фактов одной общей фразой.',
    ].join('\n')
  }
  if (profile.mode === 'balanced_source') {
    return [
      'Профиль исходных данных для описания: текст и фотографии взаимно дополняют друг друга.',
      evidence,
      'Целевой объём description: 350–650 знаков. Сохрани все конкретные факты исходника, которые не противоречат фотографиям, и добавь только видимые признаки. При нехватке данных не достраивай состав материала, технологию или скрытую конструкцию.',
    ].join('\n')
  }
  if (profile.mode === 'photo_first') {
    return [
      'Профиль исходных данных для описания: скудный текст, фотографии — основной источник.',
      evidence,
      'Целевой объём description: 250–500 знаков. Опиши видимые силуэт, форму, цвет, фактуру, конструкцию, застёжки и подошву. Не выдумывай точный состав, технологию, размеры или скрытые свойства, которых нет в тексте и нельзя подтвердить по фото.',
    ].join('\n')
  }
  return [
    'Профиль исходных данных для описания: скудный текст без достаточных фотографий.',
    evidence,
    'Пиши коротко и только по подтверждённым фактам; не заполняй описание общими рекламными фразами или догадками.',
  ].join('\n')
}
...
export function buildBatchAiVisualSetCorrectionPrompt(userPrompt: string) {
  const visualOnlyPrompt = userPrompt
    .split('\n\n')
    .filter((section) => !section.startsWith('Товар: '))
    .join('\n\n')
  return [
    visualOnlyPrompt,
    'ОБЯЗАТЕЛЬНАЯ ВИЗУАЛЬНАЯ ПЕРЕПРОВЕРКА: исходный текст товара намеренно не передан, потому что предварительный ответ назвал товар комплектом. Определи товар независимо только по приложенным contact sheet.',
    'Признавай комплектом только случай, когда на серии фотографий отчётливо видны минимум две разные вещи. Текст внутри фотографии с размерной таблицей, ценой или перечислением верха и брюк не является доказательством второй вещи. Если на всех фото показана одна вещь, укажи только её фактический тип, название, описание и подкатегорию; не упоминай отсутствующие предметы и не создавай measurements.tabs для отсутствующей вещи.',
