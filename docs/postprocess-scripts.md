...
только положительное доказательство перевыкладки: одинаковая нормализованная
полная галерея (query/hash в URL не учитываются). В такой группе сохраняется
существующий Rails-ID; если существующего ID нет, остаётся карточка с более
поздней исходной позицией. Одинаковое описание, модель или тег без одинаковой
галереи дублем не считаются, поэтому разные цвета не схлопываются.
...
Для выгрузки `9a144575-f5c6-4f62-a858-daabc1265f4a` контрольный прогон и применение дали
...
сформировано 219 семейств на 1916 карточках. В результате не осталось точных
дубликатов галерей. В начале ленты позиции `#7/#13/#16/#19/#27` получили семью
`6608 + 25x22x10.5`, а `#14/#17/#20/#28` — отдельную семью
`6608 + 32x24x12`. Итог записан в `SCRIPT_PROCESSED`; публикация в Rails не
запускалась.
