...
миграция `db:migrate:media-seo`, которая гарантирует наличие `products.slug` и
`products.photo_alts`. Поэтому открытие выгрузок не зависит от того, была ли
ручная полная миграция выполнена до redeploy.
