...
Текстовый поиск по каталогу использует два параметра Rails:

- `q` — имя, `external_id`, `sku`, `slug` и `sku` вариантов, плюс точное
  совпадение по SEO-артикулу;
- `name` — то же самое, но **без** `slug`.

Slug в каталоге не всегда соответствует текущему названию: у переименованных
товаров в нём остаётся прежнее слово модели (`Hermes 24/24` со slug
`hermes-kelly-29-2`). Поэтому обычный текст из фильтра «Название» AdminYeezy
отправляет в `name`, а `q` использует только для ссылки, пути `/product/<slug>`,
голого slug или артикула — там широкий поиск нужен и безопасен. Так запрос
«kelly» перестаёт находить товары, у которых этого слова нет в названии.
...
- **Инвалидация кеша**: при изменении имени или аватарки поставщика в «Выгрузках» автоматически инвалидируется кеш `/admin`, чтобы каталог сразу отображал актуальные данные. Дополнительно AdminYeezy держит процессный кэш справочников каталога (бренды, категории, фасеты поставщиков) на 60 секунд; писатели поставщиков и таксономии сбрасывают его явно (`invalidateRailsCatalogCaches`), поэтому обычная правка видна сразу, а TTL ограничивает расхождение для остальных случаев.
- **Фасеты поставщиков**: селектору поставщика нужен список по всему каталогу. Раньше он считался отдельной полной агрегацией `facets_batch` на каждый рендер `/admin`; теперь используется тот же процессный кэш, а страница считает фасеты только по активному фильтру.
- **Выборка 500 товаров**: чанки по 100 запрашиваются параллельно после первого ответа, который сообщает общее число страниц, вместо последовательного цикла.
