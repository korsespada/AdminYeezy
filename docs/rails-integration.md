...
### Поставщики каталога и защита от дублирования

- **Источник правды**: справочник поставщиков в `yeezy_scraping.suppliers` («Выгрузки» -> «Поставщики») является первоисточником имени, аватарки (`avatar_url`) и внешнего альбома (`album_id`). Карточки товаров (grid view, list view, table view) приоритетно берут имя и аватарку из справочника выгрузок.
- **Канонический ID**: при дублировании товара или сохранении формы, если товар уже привязан к Rails-поставщику (валидный UUID), в payload передаётся только `primary_supplier_id`. Поле `primary_supplier_name` исключается, чтобы контроллер Rails не вызывал `Supplier.find_or_create_by!(name: ...)` и не создавал дубликаты записей поставщиков.
- **Агрегация фасетов**: сайдбар фильтров агрегирует `supplierFacets` по нормализованному названию поставщика с суммированием количества товаров, предотвращая раздвоение строк в выпадающем списке.
- **Инвалидация кеша**: при изменении имени или аватарки поставщика в «Выгрузках» автоматически инвалидируется кеш `/admin`, чтобы каталог сразу отображал актуальные данные.
...
Для JSON-публикации адаптер передаёт технический `external_id`, slug товара, необязательные `video_url`/`video_poster_url` и массив `media`, где у каждого изображения есть HTTPS URL, `alt_text`, `sort_order` и `processing_status`. Rails сохраняет `external_id` только как ключ идемпотентной синхронизации. В `name` передаётся видимое название без бренда, но с цветом: бренд хранится отдельно, а цвет также дублируется в атрибутах. Rails централизованно записывает H1 равным `name`, а title и meta description формирует из бренда, названия и текущей цены, поэтому ручное изменение цены сразу обновляет title. Финальный slug Rails формирует из бренда, названия, подтверждённой модели, если её ещё нет в названии, и внутреннего SEO-артикула. Цвет отдельно в конец не добавляется, потому что он уже присутствует в `name`; длинный `external_id` не попадает ни в slug товара, ни в имя изображения. SEO-артикул присутствует в каждом slug и обеспечивает его уникальность даже для товаров с совпадающими текстовыми данными. При прямом обновлении существующего товара используется `PATCH /api/v1/admin/products/:id` с тем же набором медиа-полей. Перед таким запросом AdminYeezy преобразует категорию и подкатегорию по актуальному дереву Rails, а при отсутствии соответствия сохраняет уже назначенную Rails-категорию.

При изменении базовой цены через `PATCH /api/v1/admin/products/:id` Rails синхронизирует цену автоматически созданных размерных вариантов (`metadata.generated_from = catalog_attributes.sizes`), но не перезаписывает вручную заданные цены вариантов.

При любой смене канонического slug Rails атомарно сохраняет постоянный `301`
со старого `/product/{slug}` на новый адрес. Если slug менялся несколько раз,
все созданные этой логикой старые адреса сразу указывают на последний URL без
цепочек. Исторический slug повторно не становится каноническим: если формула
снова даёт прежний адрес, Rails добавляет порядковый номер перед SEO-артикулом
(`...-2-XXX-12345`) и направляет на него всю историю. Sitemap использует только
текущее поле `products.slug`, а
старые адреса остаются только в истории редиректов. Отдельные Chromoff slug и
переименование уже загруженных S3-изображений эта логика не затрагивает.

При назначении поставщика в карточке товара AdminYeezy передаёт в этот же
`PATCH`/import имя и аватар поставщика, а для поставщиков выгрузок — точный
`suppliers.album_id` в `metadata.source_supplier_id`. Rails переиспользует или
создаёт `Product.primary_supplier` по имени, а source ID остаётся отдельным
техническим ключом для автосинхронизации Chromoff.

При дублировании карточки в каталоге (`ProductCard`, `ProductTableView`, `ProductListItem`) формируется новый товар с `(Копия)` в названии и уникальным `productId`. Все поля оригинала полностью переносятся: бренд, категория/подкатегория, каталожные атрибуты (`catalog_attributes`, из которых Rails генерирует варианты размеров), сроки производства и доставки, SEO-данные, медиа и поставщик (`supplier_name`, `supplier_avatar`, `supplier_id`, `supplier_source_id`). В каталоге карточка выводит аватарку поставщика в правом углу строки бренда с браузерной подсказкой (title) при наведении, а фильтр поставщиков в боковой панели отображает аватарку напротив каждого поставщика.
